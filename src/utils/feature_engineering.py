...
from .api_football_utils import get_h2h
import jaro
...
def create_obs_from_match(match: Match) -> Observation:
    ...
    try:
        h2h_diff, home_gd, h2h_games = calc_h2h(match.home_team, match.away_team, match.league["id"], match.season, match.date)
    except Exception as e:
        print(e)
...
def create_standings(league_id, season):
    mfc = MongoFootballClient(conf.MONGO_URL)

    seasons_matches = mfc.get_leagues_matches(league_id, season)

    if not mfc.standing_exists(league_id, season):
        initialise_table(league_id, season)
    ...
    for batch in update_batches:
        update_table(batch, last_date)
        last_date = batch[0].date

def update_table(match_batch: List[Match], last_date):
    mfc = MongoFootballClient(conf.MONGO_URL)
    standing = mfc.get_standings(last_date, match_batch[0].league["id"], match_batch[0].season)

    standing_exists = mfc.get_standings(match_batch[0].date, match_batch[0].league["id"], match_batch[0].season)
    if standing_exists is None:
        try:
            for match in match_batch:
                home_last_games = mfc.get_last_5_games(match.league["id"], match.season, match.home_team, match.date, True)
                away_last_games = mfc.get_last_5_games(match.league["id"], match.season, match.away_team, match.date, True)
                ...
                if match.result == "Home Win":
                    ...
                    raise RuntimeError(f"Result for finished match has to be Home Win, Away Win or Draw, result was {match.result}")
                ...
                standings_history = mfc.get_last_stansings(match.league["id"], match.season, match.date)
...
def initialise_table(league_id, season):
    mfc = MongoFootballClient(conf.MONGO_URL)
